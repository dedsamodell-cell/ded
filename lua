...
ui.tabs = {
    combat = ui.window:AddTab('aimbot_hitbox'),
...
do
    ...
    Depbox1:AddSlider('aimbot_fov_size',{Text = 'target fov',Default = 67,Min = 10,Max = 6767,Rounding = 0,Compact = true,Callback = function(State)
...
do
    ...
    hbb:AddSlider('hitbox_head_size_x', { Text = 'size x', Default = 10, Min = 1, Max = 999, Rounding = 1, Compact = false }):OnChanged(function(State)
        hitboxheadsizex = State
        hbsize = _Vector3new(hitboxheadsizex, hitboxheadsizey, hitboxheadsizex)
    end)
    hbb:AddSlider('hitbox_head_size_y', { Text = 'size y', Default = 10, Min = 1, Max = 999, Rounding = 1, Compact = false }):OnChanged(function(State)
...
do
    ...
    "dedsamodellv5 canary | Jul 28 2024 | fps: 63 ";
    ...
    cheat.utility.new_renderstepped(LPH_JIT_MAX(function(delta)
        ...
        watertext = ("dedsamodellv5 %s | %s | %s | %s fps"):format(SWG_Version, SWG_ShortName, os.date("%b %d %Y"), tostring(finalfps));
...
do
    ...
    mvb:AddToggle('jetpackabuse', {Text = 'jetpack fly',Default = false,Callback = function(first)
        jetpackabuse = first
    end}):AddKeyPicker('jetpackabuse_bind', {Default = 'None',SyncToggleState = true,Mode = 'Toggle',Text = 'jetpack abuse',NoUI = false})
    mvb:AddSlider('jetpackabuse_upspeed',{ Text = 'up speed', Default = 55, Min = 5, Max = 333, Rounding = 0, Suffix = "sps", Compact = false }):OnChanged(function(State)
        abuseupspeed = State
    end)
    mvb:AddSlider('jetpackabuse_downspeed',{ Text = 'down speed', Default = 55, Min = 5, Max = 333, Rounding = 0, Suffix = "sps", Compact = false }):OnChanged(function(State)
...
do
    ...
    mvb:AddSlider('carfly_speed',{ Text = 'speed', Default = 150, Min = 20, Max = 40, Rounding = 0, Suffix = "sps", Compact = false }):OnChanged(function(State)
        speed = State
    end)
...
cheat.ThemeManager:SetFolder('dedsamodell')
cheat.SaveManager:SetFolder('dedsamodell')
